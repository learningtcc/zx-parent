...
public class BankcardBinManagerImpl extends BaseManager<BankcardBin,Long> implements IBankcardBinManager{
	...
	public BankcardBin getByCardBin(final String bankNo) {
		...
		throw new TradeException(TradeRespConstant.CARD_NO_UNSUPPORT, TradeRespConstant.CARDNO_UNSUPPORT_MSG);
	...
	private String getCardBinRedisKey(int cardNoLen,int cardBinLen,String cardBin){
		return TradeRedisConstants.KEY_PROJECT+"-"+TradeRedisConstants.KEY_MODULE_BASIC+"-"+TradeRedisConstants.KEY_BUSINESS_CARDBIN+"-"+cardNoLen+"-"+cardBinLen+"-"+cardBin;
	}
