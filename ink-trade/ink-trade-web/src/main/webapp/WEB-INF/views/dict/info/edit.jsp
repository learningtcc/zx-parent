...
<div class="title01">
	...
	<form id="form1" action="${yk}/dict/info/update.do" method="post">

		<table cellpadding="0" cellspacing="0" class="table00" style="width: 100%">
			<input type="hidden" id="id" name="id" value="<c:out value="${model.id}"/>"/>
			<input id="systermCode" name="systermCode" value="<c:out value="${model.systermCode}"/>" type="hidden"  />
			<input id="moduleCode" name="moduleCode" value="<c:out value="${model.moduleCode}"/>" type="hidden" />
			...
			<tr>
				...
				<td>
					<yk:code2name tableName="systerm_source" columnName="name" where="code=?" paramValue="${model.systermCode}" emptyTip="${model.systermCode}"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<yk:code2name tableName="systerm_module" columnName="name" where="code=?" paramValue="${model.moduleCode}" emptyTip="${model.moduleCode}"/>
				</td>
			</tr>
			<tr>
				<td class="txtr" style="width: 188px">
			...
			<tr>	
				...
				<td>
					<select id="status" name="status" class="select02">
...
$(function(){
	...
	$("#status").val('<c:out value="${model.status}"/>');

	$("#subSave").click(function() {
		if (validateLengthFrom("name", 1, "业务模块名不能为空")) {
			$('#form1').submit();
		}
	});
});
