...
<body>
	...
	<table cellpadding="0" cellspacing="0" class="table00" style="width: 100%">
		...
		<tr>	
			...
			<td>
				<c:choose><c:when test="${model.status==0}">正常</c:when><c:when test="${model.status==1}">停用</c:when><c:otherwise>未知</c:otherwise></c:choose>
			</td>
		</tr>
		<tr>	
		...
		<tr>	
			...
			<td><yk:code2name tableName="monitor_user" columnName="fullName" where="userid=?" paramValue="${model.opUser}" emptyTip="${model.opUser}"/></td>
		</tr>
		<tr>	
