...
//		PrivateKey privateKey = RsaReadUtil.getPrivateKeyFromFile(pfxPath, priKeyPass);
		privateKey=getPrivateKey(pfxPath,priKeyPass);
	    System.out.println("1----根据私钥加密之后的KEY:"+privateKey);
		if (privateKey == null) {
			return null;
		}
...
//		PublicKey publicKey = RsaReadUtil.getPublicKeyFromFile(pubCerPath);
	    publicKey=getPublicKey(pubCerPath);
		if (publicKey == null) {
			return null;
		}
	 ...
	 * @return
	 */
	private static PrivateKey getPrivateKey(String pfxPath, String priKeyPass){
	    if(privateKey==null){
	        synchronized (RsaCodingUtil.class) {
                if(privateKey==null){
                   privateKey= RsaReadUtil.getPrivateKeyFromFile(pfxPath, priKeyPass);  
                }
            }
	    }
	    return privateKey;
	 ...
	 * @return
	 */
	private static PublicKey getPublicKey(String pubCerPath){
	    if(publicKey==null){
            synchronized (RsaCodingUtil.class) {
                if(publicKey==null){
                    publicKey= RsaReadUtil.getPublicKeyFromFile(pubCerPath);  
                }
            }
        }
        return publicKey;
