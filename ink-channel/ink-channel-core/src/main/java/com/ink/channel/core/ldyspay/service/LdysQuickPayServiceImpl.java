 ...
 *
 */
@Service("ldysQuickPayService")
public class LdysQuickPayServiceImpl implements AsileQuickPayService{
	private static YinkerLogger logger = YinkerLogger.getLogger(LdysQuickPayServiceImpl.class);
	...
	public AsileQuickPayOutput pay(AsileQuickPayInput input) {
		logger.info(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.LDYS_QUICK_PAY_CODE,"开始调用联动优势首次支付接口");
		...
		try {
			...
			logger.info(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_PAY_CODE, "http请求参数:"+map.toString());
			Map<String, String> returnMap = LdysMessEncryptUtils.messEncrypt(map, input.getMerchantNo());
			logger.info(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_PAY_CODE, "http响应结果:"+returnMap.toString());
			...
			if(LdysConstant.decodeCode.equals(exceptionCode)){
				...
				out.setResMsg("联动优势首次支付解密验签失败");
				out.setOrderNo(input.getOrderNo());
				out.setResStatus(OrderStatus.PROCESSES_CODE.getCode());//处理中
				logger.error(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_PAY_CODE, "联动优势首次支付解密验签失败",null);
			}else{
				out=new AsileQuickPayOutput();
			...
			out.setOrderNo(input.getOrderNo());
			out.setResStatus(OrderStatus.PROCESSES_CODE.getCode());//处理中
			logger.error(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_PAY_CODE, e.getMessage(),e,null);
		}
		logger.info(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.LDYS_QUICK_PAY_CODE,"结束调用联动优势首次支付接口");
		return out;
	}
