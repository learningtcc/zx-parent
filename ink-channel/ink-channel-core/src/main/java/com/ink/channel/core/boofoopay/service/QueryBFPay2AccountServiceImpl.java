 ...
 *
 */
@Service("queryBFPay2AccountService")
public class QueryBFPay2AccountServiceImpl implements AsilePay2AccountQueryService{
	private static YinkerLogger logger = YinkerLogger.getLogger(QueryBFPay2AccountServiceImpl.class);
	@Autowired
	...
	public AsileQueryPayAccountOut queryPayOrder(AsileQueryPayAccountIn queryIn) {
		...
		try{
			logger.info(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.BFOO_QUERY_PAY_ACCOUNT_CODE,"开始调用宝付查询代收接口");
			...
			QueryBFPay2AccountResp resp=BooFooRequest.excuteRequest2Account(queryIn.getMerchantNo(),req, BoofooConstant.QUERYPAY2ACCOUNT_URL, QueryBFPay2AccountResp.class);
			if(resp==null){
		...
		logger.info(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.BFOO_QUERY_PAY_ACCOUNT_CODE,"结束调用宝付查询代收接口");
		return null;
	}
