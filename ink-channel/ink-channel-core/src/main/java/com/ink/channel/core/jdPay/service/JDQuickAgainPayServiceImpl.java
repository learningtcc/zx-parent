 ...
 *
 */
@Service("jDQuickAgainPayServiceImpl")
public class JDQuickAgainPayServiceImpl implements AsileQuickAgainPayService{
	private static YinkerLogger logger = YinkerLogger.getLogger(JDQuickAgainPayServiceImpl.class);
	@Autowired
	...
	public AsileQuickPayOutput againPay(AsileQuickAgainPayInput input) {
		logger.info(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.JD_QUICK_AGAIN_PAY_CODE,"进入京东快捷支付再次支付，订单号：" + input.getOrderNo());
		...
		try {
			...
			logger.info(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.JD_QUICK_AGAIN_PAY_CODE,"京东快捷支付再次支付HTTP请求参数："+params.toString());
			Map<String, Object> returnMap=JDHttpClientUtils.trade(params,input.getMerchantNo());
			logger.info(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.JD_QUICK_AGAIN_PAY_CODE,"京东快捷支付再次支付HTTP响应结果："+returnMap.toString());
			...
			if(LdysConstant.decodeCode.equals(exceptionCode)){
				...
				out.setResMsg("京东快捷再次支付解密验签失败");
				out.setOrderNo(input.getOrderNo());
				out.setResStatus(OrderStatus.PROCESSES_CODE.getCode());//处理中
				logger.error(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.JD_QUICK_AGAIN_PAY_CODE, "京东快捷支付再次支付解密验签失败",null);
			}else{
				...
				if(SystemCodeEnums.JD_SUCCESS_CODE.getCode().equals(resCode)){
					try{
						...
						out.setIdentityId(input.getIdentityId());
					}catch(Exception ex){
						out.setResStatus(OrderStatus.PROCESSES_CODE.getCode());//处理中
						logger.error(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.JD_QUICK_AGAIN_PAY_CODE, ex.getMessage(),ex,null);
					}
			...
			logger.error(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.JD_QUICK_AGAIN_PAY_CODE, "京东快捷支付再次支付"+e.getMessage(), e,null);
			e.printStackTrace();
		}
		return out;
	}
