...
@Service("ldysQuickAgainValidService")
public class LdysQuickAgainValidServiceImpl implements AsileQuickAgainValidCodeService{
	private static YinkerLogger logger = YinkerLogger.getLogger(LdysQuickAgainValidServiceImpl.class);
	...
	public AsileQuickAgainValidCodeOutput quickAgainValidCode(AsileQuickAgainValidCodeInput input) {
		logger.info(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.LDYS_QUICK_AGAIN_VALID_CODE,"开始调用联动优势再次支付发短验接口");
		AsileQuickAgainValidCodeOutput out=null;
		...
		try {
			...
			String channeID = certificateUtil.getIpMapsConfig().get(LdysConstant.CHANNELID);
			map.put("notify_url",certificateUtil.getIpMapsConfig().get(channeID+LdysConstant.NOTIFY_URL));//服务器异步通知页面路径
			...
			logger.info(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_AGAIN_VALID_CODE, "http请求参数:"+map.toString());
			Map<String, String> returnMap = LdysMessEncryptUtils.messEncrypt(map, input.getMerchantNo());
			logger.info(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_AGAIN_VALID_CODE, "http响应结果:"+returnMap.toString());
			String exceptionCode = returnMap.get("exceptionCode");
			if(LdysConstant.decodeCode.equals(exceptionCode)){
				//解密异常
				out=new AsileQuickAgainValidCodeOutput();
				out.setResCode(SystemCodeEnums.LDYS_FAILE_CODE.getCode());
				out.setResMsg("联动优势再次支付发短验解密验签失败");
				out.setOrderNo(input.getOrderNo());
				logger.error(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_AGAIN_VALID_CODE, "联动优势再次支付发短验解密验签失败",null);
			}else{
				//无异常
				out=new AsileQuickAgainValidCodeOutput();
			...
			logger.error(ChannelConstants.LOGGER_MODULE_NAME,ChannelConstants.LDYS_QUICK_AGAIN_VALID_CODE, e.getMessage(),e,null);
		} 
		logger.info(ChannelConstants.LOGGER_MODULE_NAME, ChannelConstants.LDYS_QUICK_AGAIN_VALID_CODE,"结束调用联动优势再次支付发短验接口");
		return out;
	  }
}