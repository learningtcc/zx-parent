...
<form id="queryForm" name="queryForm" action="<c:url value="/job/list.do"/>" method="post" style="display: inline;">
	...
	<h2><%=TimerTaskJob.TABLE_ALIAS%>列表</h2>
		...
		<div>
     	 ...
     	 <yk:page url="${yk}/job/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/job/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/job/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function pause(id){
	if(id != null && confirm("确定要暂停吗？")){
		$.post("${yk}/job/pause.do",{jobId:id}, function(data) {
			if(data.result == "true"){
				//暂停后页面处理
				alert('暂停成功');
				$('#queryForm').submit();
...
function resume(id){
	if(id != null && confirm("确定要暂停吗？")){
		$.post("${yk}/job/resume.do",{jobId:id}, function(data) {
            if(data.result == "true"){
				//恢复后页面处理
				alert('恢复成功');
				$("#queryBtn").click();
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/job/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/job/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/job/delete.do", {"items":ids.join(',')},
				function(data){
                    if(data.result == "true"){
						alert('删除成功');
						$('#queryForm').submit();
