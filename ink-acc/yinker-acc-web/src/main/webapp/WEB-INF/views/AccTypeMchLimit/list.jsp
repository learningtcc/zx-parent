...
<head>
	<title><%=AccTypeMchLimit.TABLE_ALIAS%> 维护</title>
...
<form id="queryForm" name="queryForm" action="<c:url value="/AccTypeMchLimit/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>	
			...
			<td>
				<yk:select codeName="sac_name" table="acc_sac_type" codeValue="sac_type" id="sacType" name="sacType" select="${sacType }" where=" status = 1 and sac_type < '0011' "></yk:select>
			</td>
	...
	<br></br>
	<div class="title01">
	<h2><%=AccTypeMchLimit.TABLE_ALIAS%>列表</h2>
</div>
		<div class="groupbtn">
		...
		<div>
			...
			<tr>
				...
				<td>
				<div>
				<yk:code2name columnName="sac_name" tableName="acc_sac_type" where=" sac_type=? " paramValue="${entry.sacType}"></yk:code2name></div>
				</td>
				<td>
				<div>
				<yk:code2name columnName="mch_name" tableName="acc_mch" where=" mch_id=? " paramValue="${entry.mchId}"></yk:code2name></div>
				</td>
				<td>
