...
<body>
	...
	<table cellpadding="0" cellspacing="0" class="table00"
		...
		<tr>
			...
			<td><c:choose>
					<c:when test="${'CF2000027305' eq model.merchantNo}">
               		      民生-简理财
                     </c:when>
					<c:otherwise>
						<c:out value="${model.merchantNo}" />
					</c:otherwise>
				</c:choose></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><c:out value="${model.receivedAmt}" /></td>
		</tr>
		<tr>
			<td class="txtr" style="width: 188px">交易时间:</td>
			<td><fmt:formatDate value="${model.transTime}"
					pattern="yyyy-MM-dd HH:mm:ss" /></td>
		</tr>
		<tr>
			<td class="txtr" style="width: 188px">交易状态:</td>
			<td><c:choose>
					<c:when test="${model.transStatus==1}">
            				待支付
     					</c:when>
					<c:when test="${model.transStatus==2}">
           			 		支付成功
    					</c:when>
					<c:when test="${model.transStatus==3}">
           			 		支付失败
    					</c:when>
					<c:otherwise>
            				状态差错
		...
		<tr>
			...
			<td><c:choose>
					<c:when test="${model.payMethod==1}">
            				代收
     					</c:when>
					<c:when test="${model.payMethod==2}">
           			 		代付
    					</c:when>
					<c:when test="${model.payMethod==3}">
           			 		网银
    					</c:when>
					<c:when test="${model.payMethod==3}">
           			 		快捷
