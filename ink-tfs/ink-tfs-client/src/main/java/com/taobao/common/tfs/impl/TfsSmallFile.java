...
public class TfsSmallFile extends TfsFile {
    ...
    public int write(byte[] data, int offset, int length) throws TfsException {
        ...
        int left = length;
        while (left > 0) {
            int writeLengthPerTime = left > TfsFile.MAX_SMALL_IO_LENGTH ? TfsFile.MAX_SMALL_IO_LENGTH : left;
    ...
    public int unlink(int action) throws TfsException {
        if (!isOpen) {
            throw new TfsException("file not opened.");
        }
    ...
    private int unlink(String mainName, String suffix, int action)
        throws TfsException {
    ...
    public String rename(String mainName, String suffix, String newSuffix)
        throws TfsException {
        FSName fsName = new FSName(mainName, suffix);
        int blockId = fsName.getBlockId();
        long oldFileId = fsName.getFileId();

        open(blockId, oldFileId, TfsConstant.UNLINK_MODE);
        fsName.setSuffix(newSuffix);
        if (newSuffix == null || newSuffix.length() == 0)
            fsName.setSuffix(0);

        long newFileId = fsName.getFileId();
        session.renameFile(blockId, oldFileId, newFileId, optionFlag, dsListWrapper);
        fsName.setFileId(newFileId);
    ...
    public String rename(String tfsName, String newSuffix)
        throws TfsException {
        return rename(tfsName, null, newSuffix);
    }
    ...
    private String saveUniqueToTair(UniqueStore uniqueStore, byte[] key, UniqueValue value) {
        ...
        try {
            ...
            if (valueRet.getReferenceCount() != TfsConstant.UNIQUE_INSERT_VERSION_ERROR) {
    ...
    public String saveUnique(UniqueStore uniqueStore, String mainName,
        ...
        if (data == null || offset < 0 || length < 0) {
            throw new TfsException("invalid argument: data: " + data + ", length:" + length
                                   + ", offset:" + offset + " illegal.");
        }
        if (data.length < offset + length) {
            throw new TfsException("invalid argument: write data length: " + data.length +
                                   " less than input: " + (offset + length));
        }
        ...
        if ((reSave & 0x1) != 0) {
            ...
            retName = getFileName(simpleName) + (null == suffix ? "" : suffix);
        }
        ...
        if ((reSave & 0x2) != 0) {
            ...
            // if not first save and need return simpleName type,
            // then convert stored name in tair to simpleName
            if ((reSave & 0x1) == 0 && simpleName) {
                FSName fsname = new FSName(retName);
                retName = fsname.get(false, simpleName) + (null == suffix ? "" : suffix);
            }
        }
     ...
     * @exception
     */
    public long unlinkUnique(UniqueStore uniqueStore, FSName fsNameCurr) throws TfsException {
        FileInfo info = stat();

        if (info.getFlag() == TfsConstant.NORMAL_STATUS ||
            info.getFlag() == TfsConstant.HIDE_STATUS ) {
            ...
            do {
                ...
                String tfsNameExist = value.getTfsName();
                FSName fsNameExist = new FSName(tfsNameExist);
                // check file to make sure unlink the right file
                if (fsNameCurr.getBlockId() == fsNameExist.getBlockId() &&
                    fsNameCurr.getFileId() == fsNameExist.getFileId()) {
                    ...
                    // curr file name not exist in tair, just unlink
                    this.mode = TfsConstant.UNLINK_MODE;
                    unlink(UnlinkFileMessage.DELETE);
                    break;
                }
    ...
    public long unlinkUnique(UniqueStore uniqueStore, String mainName, String suffix) throws TfsException {
        open(mainName, suffix, TfsConstant.READ_MODE);
        FSName fsName = new FSName(mainName, suffix);
        return unlinkUnique(uniqueStore, fsName);
    }

    public long unlinkUnique(UniqueStore uniqueStore, String tfsName) throws TfsException {
        open(tfsName, null, TfsConstant.READ_MODE);
        FSName fsName = new FSName(tfsName, null);
        return unlinkUnique(uniqueStore, fsName);
    }
