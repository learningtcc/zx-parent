...
public class TfsUtil {
    private static final int _crc32tab[] = {
    ...
    public static void checkMalloc(int size) {
        if (size > TfsConstant.TFS_MALLOC_MAX) {
            throw new IllegalArgumentException("alloc to too large byte[], size: " + size);
        }
    }
    ...
    public static String longToHost(long id) {
        return idToAddress(id).toString();
    }
    ...
    public static int crc32(int crc, byte[] data, int offset, int length) {
        for (int i = offset; i < offset + length; ++i) {
            crc = (crc >>> 8) ^ _crc32tab[(crc ^ (data[i])) & 0xff];
        }
        return crc;
    }

    public static void createDirectory(String directory, String mode) throws TfsException {
        File directoryPath = new File(directory);

        if (!directoryPath.exists()) {
            if (!directoryPath.mkdirs()) {
                throw new TfsException("create directory fail: " + directoryPath.getAbsolutePath());
            }

            if (TfsConstant.TFS_TMP_PATH_NEED_CHMOD) {
                try {
                    Runtime.getRuntime().exec(TfsConstant.CHMOD_CMD + " " + mode + " " +
            ...
            throw new TfsException("path exist but not directory: " + directoryPath.getAbsolutePath());
        }
    }
