...
public abstract class TfsFile {
    ...
    protected void open(int mode) throws TfsException {
        ...
        if ((mode & TfsConstant.READ_MODE) != 0) {
            if ((mode & TfsConstant.FORCE_MODE) != 0) {
                setReadFlag((byte)FORCE_READ);
            } else {
                setReadFlag((byte)NORMAL_READ);
            }
        }
    ...
    public abstract long unlinkUnique(UniqueStore uniqueStore, String tfsName)
        throws TfsException;
