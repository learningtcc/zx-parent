...
public class TairUniqueStore implements UniqueStore {
    ...
    public UniqueValue decrement(byte[] key, UniqueValue value) throws TfsException {
        if (value.getReferenceCount() <= 0) {
            ...
            // uncomment this line, (should throw exception and comment the "return" part) for normal use
            //throw new TfsException("Referencecount invalid error: " + value.getReferenceCount());
            return value;
        }
        value.subRef(1);
        try {
            // CAUTION: comment this "if else" block for histroy problem, avoid unlinking file on line,
            // uncomment them for normal use
            //if (value.getReferenceCount() == 0) {
            //    return delete (key);
            //} else {
