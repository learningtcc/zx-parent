...
<form id="queryForm" name="queryForm" action="<c:url value="/user/AccCust/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		...
		<tr>
			...
			<td>
				<yk:select codeName="mch_name" table="acc_mch" codeValue="mch_id" id="mchId" name="mchId" select="${mchId }" where=" del_flag = 0" serviceName="userDubboBaseService"></yk:select>
			</td>	
			<td class="txtr" style="width: 100px">客户号：</td>		
			<td>
				<input value="<c:out value="${custId}"/>" id="custId" name="custId" maxlength="64"/>
			</td>
		</tr>	
	...
	<h2>客户列表</h2>
		...
		<div>
     	 ...
     	 <yk:page url="${yk}/user/AccCust/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/user/AccCust/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/user/AccCust/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/user/AccCust/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/user/AccCust/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/user/AccCust/delete.do", {"items":ids.join(',')},
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
