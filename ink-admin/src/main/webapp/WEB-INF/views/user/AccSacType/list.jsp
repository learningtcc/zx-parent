...
<form id="queryForm" name="queryForm" action="<c:url value="/user/AccSacType/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>	
			...
			<td>
				<yk:select codeName="sac_name" table="acc_sac_type" codeValue="sac_type" id="sacType" name="sacType" select="${sacType }" where=" status = 1" serviceName="userDubboBaseService"></yk:select>
			</td>
			<td class="txtr" style="width: 100px">子帐户名称：</td>		
			<td>
				<input value="<c:out value="${sacName}"/>" id="sacName" name="sacName" maxlength="64"/>
			</td>
	...
	<h2>子账户列表</h2>
		...
		<div class="groupbtn">
			...
			<a href="javascript:void(0)" title="删除"  class="btn004" id="btndelete">注销</a> -->
		</div>
		<div>
			...
			<thead>
			 <tr>
				...
				<th>子帐户名称<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>币种<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
			...
			<tr>
				...
				<td>
				<div><c:out value="${entry.sacName}"/></div>
				</td>
				<td>
				<div><c:out value="${entry.cur}"/></div>
				</td>
				<td>
				<div>
					<c:choose>
						...
						<c:when test="${entry.status == 3 }">待审核</c:when>
						<c:when test="${entry.status == 4 }">审核拒绝</c:when>
     	 ...
     	 <yk:page url="${yk}/user/AccSacType/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/user/AccSacType/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/user/AccSacType/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/user/AccSacType/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/user/AccSacType/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	...
	$('#btnclear').click(function(){
			...
			$('#cur').val('');
			$('#upItemId').val('');
			$('#dayVarchargelmtAmt').val('');
			$('#dayVarchargelmtCnt').val('');
			$('#dayPaylmtAmt').val('');
			$('#dayPaylmtCnt').val('');
			$('#dayInlmtAmt').val('');
			$('#dayInlmtCnt').val('');
			...
			$('#dayCashlmtAmt').val('');
			$('#dayCashlmtCnt').val('');
			$('#monVarchargelmtAmt').val('');
			$('#monVarchargelmtCnt').val('');
			$('#monPaylmtAmt').val('');
			$('#monPaylmtCnt').val('');
			$('#monInlmtAmt').val('');
			$('#monInlmtCnt').val('');
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/user/AccSacType/delete.do", {"items":ids.join(',')},
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
