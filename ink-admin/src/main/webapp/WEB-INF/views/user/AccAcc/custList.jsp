...
<form id="queryForm" name="queryForm" action="<c:url value="/user/AccAcc/custList.do"/>" method="post" style="display: inline;">
		...
		<div>
			...
			<thead>
			 <tr>
				...
				<th>最后修改<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
			...
			<tr>
				...
				<td>
				<div><yk:code2name columnName="sac_name" tableName="acc_sac_type" where=" sac_type=? " paramValue="${entry.sacType}" serviceName="userDubboBaseService"></yk:code2name></div>
				</td>
				<td>
     	 ...
     	 <yk:page url="${yk}/user/AccAcc/custList.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/user/AccAcc/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/user/AccAcc/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/user/AccAcc/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/user/AccAcc/delete.do", {"items":ids.join(',')},
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
