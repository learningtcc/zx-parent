...
<form id="queryForm" name="queryForm" action="<c:url value="/monitor/dict/source/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>	
			<td class="txtr" style="width: 100px">平台系统名：</td>
			<td>
				&nbsp;<input value="<c:out value="${name}"/>" id="name" name="name" maxlength="20"/>
			</td>
			<td class="txtr" style="width: 100px">平台系统码：</td>
			<td>
		...
		<div>
			...
			<thead>
			 <tr>
				...
				<th>平台系统名<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>平台系统码<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>节点数<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>Ip列表<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
			...
			<tr>
				...
				<td>
				<div><c:out value="${entry.count}"/></div>
				</td>
				<td>
				<div><c:out value="${entry.ipList}"/></div>
				</td>
				<td>
				<div>
				...
				<td>
				<div><yk:code2name tableName="monitor_user" columnName="fullName" where="userid=?" paramValue="${entry.opUser}" emptyTip="${entry.opUser}" serviceName="monitorDubboBaseService"/></div>
				</td>
				<td>
     	 ...
     	 <yk:page url="${yk}/monitor/dict/source/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/monitor/dict/source/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/monitor/dict/source/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/monitor/dict/source/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/monitor/dict/source/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids,status){
	...
	if(ids.length>0 && confirm("确定要"+showStr+"吗？")){
		$.post( "${yk}/monitor/dict/source/delete.do", {"items":ids.join(','),"status":status},
				function(data){
					if(data=='1'){
						alert(showStr+'成功');
						$('#queryForm').submit();
