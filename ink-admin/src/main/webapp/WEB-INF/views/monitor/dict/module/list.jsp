...
<form id="queryForm" name="queryForm" action="<c:url value="/monitor/dict/module/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>	
			...
			<td class="txtr" style="width: 100px">功能模块名称：</td>
			<td>
				&nbsp;<input value="<c:out value="${name}"/>" id="name" name="name" maxlength="20"/>
			</td>
			<td class="txtr" style="width: 100px">功能模块码：</td>
			<td>
		...
		<div>
			...
			<thead>
			 <tr>
				...
				<th>功能模块名称<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>功能模块码<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
			...
			<tr>
				...
				<td>${v.index + 1 }</td>
				<td>
				<div><yk:code2name tableName="systerm_source" columnName="name" where="code=?" paramValue="${entry.systermCode}" emptyTip="${entry.systermCode}" serviceName="monitorDubboBaseService"/></div>
				</td>
				<td>
     	 ...
     	 <yk:page url="${yk}/monitor/dict/module/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/monitor/dict/module/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/monitor/dict/module/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/monitor/dict/module/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/monitor/dict/module/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids,status){
	...
	if(ids.length>0 && confirm("确定要"+showStr+"吗？")){
		$.post( "${yk}/monitor/dict/module/delete.do", {"items":ids.join(','),"status":status},
				function(data){
					if(data=='1'){
						alert(showStr+'成功');
						$('#queryForm').submit();
