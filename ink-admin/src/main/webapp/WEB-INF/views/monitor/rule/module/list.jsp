...
<form id="queryForm" name="queryForm" action="<c:url value="/monitor/rule/module/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>	
			...
			<td>
				<%--&nbsp;<input value="<c:out value="${moduleCode}"/>" id="moduleCode" name="moduleCode" maxlength="20"/>--%>
	...
	<h2>监控列表</h2>
		...
		<div>
     	 ...
     	 <yk:page url="${yk}/monitor/rule/module/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/monitor/rule/module/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/monitor/rule/module/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/monitor/rule/module/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/monitor/rule/module/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	...
	$("#sysCode").change(function(){
		...
		executeAjax(moduleUrl,"moduleCode",'<c:out value="${moduleCode}"/>');
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/monitor/rule/module/delete.do", {"items":ids.join(',')},
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
