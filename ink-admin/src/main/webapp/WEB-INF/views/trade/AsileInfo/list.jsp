...
<form id="queryForm" name="queryForm" action="<c:url value="/trade/AsileInfo/list.do"/>" method="post" style="display: inline;">
		...
		<div>
			...
			<thead>
			 <tr>
				...
				<th>渠道简码<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<!-- <th>asileBankType<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>费率名称<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>费率编号<div style="width: 65px; height: 1px; overflow: hidden;"></div></th> -->
<!-- 				<th>渠道状态<div style="width: 65px; height: 1px; overflow: hidden;"></div></th> -->
				<!-- <th>transType<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>asileProductCode<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>asileProductName<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>asileAuditStatus<div style="width: 65px; height: 1px; overflow: hidden;"></div></th> -->
				...
				<!-- <th>isDelete<div style="width: 65px; height: 1px; overflow: hidden;"></div></th> -->
			...
			<tr>
				...
				<td>
				<div><c:out value="${entry.asileFeeName}"/></div>
				</td>
				<td>
				<div><c:out value="${entry.asileFeeNo}"/></div>
				</td> --%>
<!-- 				<td> -->
<%-- 				<div><c:out value="${entry.asileStatus}"/></div> --%>
<!-- 				</td> -->
				<%-- <td>
				...
				<td>
				<div><c:out value="${entry.asileAuditStatus}"/></div>
				</td>--%>
				<td>
     	 ...
     	 <yk:page url="${yk}/trade/AsileInfo/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/trade/AsileInfo/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/trade/AsileInfo/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/trade/AsileInfo/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/trade/AsileInfo/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/trade/AsileInfo/delete.do", {"items":ids.join(',')}, 
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
