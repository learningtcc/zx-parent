...
<form id="queryForm" name="queryForm" action="<c:url value="/trade/MchBank/list.do"/>" method="post" style="display: inline;">
	...
	<h2>MchBank列表</h2>
		...
		<div>
     	 ...
     	 <yk:page url="${yk}/trade/MchBank/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/trade/MchBank/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/trade/MchBank/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/trade/MchBank/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/trade/MchBank/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/trade/MchBank/delete.do", {"items":ids.join(',')}, 
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
