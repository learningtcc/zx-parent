...
<body>
	...
	<form id="queryForm" name="queryForm"
		action="<c:url value="/trade/AsileBankTemp/list.do"/>" method="post"
...
<!-- 					</td> -->
		...
		<yk:page url="${yk}/trade/AsileBankTemp/list.do" id="logForm"
	...
	<script type="text/javascript">
		function show(id) {
			window
					.open('${yk}/trade/AsileBankTemp/show.do?' + $('#pkName').val()
							+ '=' + id, '',
							'height=600,width=600,resizable=yes,location=no,scrollbars');
		}
		function edit(id) {
			window
					.open('${yk}/trade/AsileBankTemp/edit.do?' + $('#pkName').val()
							+ '=' + id, '',
							'height=600,width=600,resizable=yes,location=no,scrollbars');
		}
		...
		$(function() {
			...
			$('#btnedit')
					.click(
							function() {
								...
								window
										.open('${yk}/trade/AsileBankTemp/edit.do?'
												+ $('#pkName').val() + '='
												+ $id.val(), '',
												'height=600,width=600,resizable=yes,location=no,scrollbars');
							});
			...
			$('#btnadd')
					.click(
							function() {
								window
										.open('${yk}/trade/AsileBankTemp/create.do',
												'',
												'height=600,width=600,resizable=yes,location=no,scrollbars');
							});
		...
		function v_deleteItems(ids) {
			if (ids.length > 0 && confirm("确定要删除吗？")) {
				$.post("${yk}/trade/AsileBankTemp/delete.do", {
					"items" : ids.join(',')
				}, function(data) {
					if (data == '1') {
						alert('删除成功');
