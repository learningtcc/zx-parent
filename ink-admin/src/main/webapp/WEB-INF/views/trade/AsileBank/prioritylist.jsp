...
<body>
	...
	<form id="queryForm" name="queryForm"
		action="<c:url value="/trade/AsileBank/prioritylist.do"/>" method="post"
...
<!-- 						href="#" class="top">置顶</a></td> -->
		...
		<yk:page url="${yk}/trade/AsileBank/prioritylist.do" id="logForm"
	...
	<script type="text/javascript">
		function show(id) {
			window
					.open('${yk}/trade/AsileBank/show.do?' + $('#pkName').val() + '='
							+ id, '',
							'height=600,width=600,resizable=yes,location=no,scrollbars');
		}
		function edit(id) {
			window
					.open('${yk}/trade/AsileBank/edit.do?' + $('#pkName').val() + '='
							+ id, '',
							'height=600,width=600,resizable=yes,location=no,scrollbars');
		}
		...
		$(function() {
			...
			$('#btnedit')
					.click(
							function() {
								...
								window
										.open('${yk}/trade/AsileBank/edit.do?'
												+ $('#pkName').val() + '='
												+ $id.val(), '',
												'height=600,width=600,resizable=yes,location=no,scrollbars');
							});
			...
			$('#btnadd')
					.click(
							function() {
								window
										.open('${yk}/trade/AsileBank/create.do', '',
												'height=600,width=600,resizable=yes,location=no,scrollbars');
							});
		...
		function v_deleteItems(ids) {
			if (ids.length > 0 && confirm("确定要删除吗？")) {
				$.post("${yk}/trade/AsileBank/delete.do", {
					"items" : ids.join(',')
				}, function(data) {
					if (data == '1') {
						alert('删除成功');
...
// 		}
	...
	<script type="text/javascript">
		...
		function priority() {
			...
			var asileBankJson=JSON.stringify(asileBanks);
			$.ajax({
				url : '${yk}/trade/AsileBank/degree.do',
				data :{asileBankJson:asileBankJson},
				dataType : "json",
				type : "POST",
				success : function(responseJSON) {
