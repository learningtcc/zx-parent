...
<form id="queryForm" name="queryForm" action="<c:url value="/trade/AuthChannelPriority/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>	
			...
			<td>
				&nbsp;
				<yk:constantConvert htmlTag="select" dataTag="" data="${asileData}" paramValue="${channelNo}" emptyTip=" " codeTag="asileCode" nameTag="asileName" name="channelNo"/>
				<%-- <input value="<c:out value="${channelNo}"/>" id="channelNo" name="channelNo" maxlength="20"/> --%>
			</td>
			<%--
			<td class="txtr" style="width: 100px">优先级，数值越小，优先级越高：</td>		
			 <td>
				&nbsp;<input value="<c:out value="${priority}"/>" id="priority" name="priority" maxlength="10"/>
			</td>
		...
		<%-- <tr>	
			<td class="txtr" style="width: 100px">操作者：</td>		
			<td>
				&nbsp;<input value="<c:out value="${operator}"/>" id="operator" name="operator" maxlength="45"/>
			</td>
		</tr> --%>	
	</table>
	<br/>
		...
		<div>
			...
			<thead>
			 <tr>
				...
				<th>优先级，数值越小，优先级越高<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				...
				<th>操作者<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
     	 ...
     	 <yk:page url="${yk}/trade/AuthChannelPriority/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/trade/AuthChannelPriority/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/trade/AuthChannelPriority/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/AuthChannelPriority/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/AuthChannelPriority/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/AuthChannelPriority/delete.do", {"items":ids.join(',')}, 
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
