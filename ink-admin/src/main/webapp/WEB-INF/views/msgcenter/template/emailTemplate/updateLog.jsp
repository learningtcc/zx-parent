...
<form id="queryForm" name="queryForm" action="<c:url value="/msgcenter/EmailTemplateLog/updateLog.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		...
		<tr>	
			...
			<td class="txtr" style="width: 100px">模板类型：</td>		
			<td>
				<select id="tempType" name="tempType">
					<option value="">不限</option>
					<option value="1" <c:if test="${tempType == '1'}">selected="selected"</c:if>>普通模板</option>
					<option value="2" <c:if test="${tempType == '2'}">selected="selected"</c:if>>营销模板</option>
				</select>
			</td>
			...
			<td>
				<select id="parseMethod" name="parseMethod">
					<option value="">不限</option>
					<option value="1" <c:if test="${parseMethod == '1'}">selected="selected"</c:if>>顺序解析</option>
					<option value="2" <c:if test="${parseMethod == '2'}">selected="selected"</c:if>>Freemarker解析</option>
				</select>
			</td>	
		...
		<tr>
			<td class="txtr" style="width: 100px">操作时间：</td>		
			<td>
				&nbsp;<input onClick="WdatePicker()" value="<c:out value="${operateTimeBegin}"/>" id="operateTimeBegin" name="operateTimeBegin" size="10"/>
				<input onClick="WdatePicker()" value="<c:out value="${operateTimeEnd}"/>" id="operateTimeEnd" name="operateTimeEnd" size="10"/>
			</td>
		</tr>	 -->
	</table>
	<br/>
	...
	<br></br>
	<div class="title01">
	<h2><%=EmailTemplateLog.TABLE_ALIAS%>列表</h2>
</div>
		<div class="groupbtn">
		...
		<div>
			...
			<thead>
			 <tr>
				...
				<th>模板类型<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
			...
			<tr>
				...
				<td>
				<div>
					<yk:code2name columnName="temp_name" tableName="email_template" where="id = ?" paramValue="${entry.templateId}" serviceName="msgcenterDubboBaseService"/>
				</div>
				</td>
				<td>
				<div>
				...
				<div>
					<c:choose>
						<c:when test="${entry.tempType == 1}">普通模板</c:when>
						<c:when test="${entry.tempType == 2}">营销模板</c:when>
				...
				<div>
					<c:choose>
						<c:when test="${entry.parseMethod == 1}">顺序解析</c:when>
						<c:when test="${entry.parseMethod == 2}">Freemarker解析</c:when>
						<c:otherwise>未定义</c:otherwise>
					</c:choose>
				</div>
				</td>
     	 ...
     	 <yk:page url="${yk}/msgcenter/EmailTemplateLog/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/msgcenter/EmailTemplateLog/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/msgcenter/EmailTemplateLog/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/msgcenter/EmailTemplateLog/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/msgcenter/EmailTemplateLog/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/msgcenter/EmailTemplateLog/delete.do", {"items":ids.join(',')},
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
