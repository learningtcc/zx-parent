...
<head>
	<title><%=SmsTemplate.TABLE_ALIAS%> 维护</title>
...
<form id="queryForm" name="queryForm" action="<c:url value="/msgcenter/SmsTemplate/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>
			...
			<td>
				<input value="<c:out value="${id}"/>" id="id" name="id" maxlength="30"/>
			</td>
	...
	<br></br>
	<div class="title01">
	<h2><%=SmsTemplate.TABLE_ALIAS%>列表</h2>
</div>
		<div class="groupbtn">
		...
		<div>
     	 ...
     	 <yk:page url="${yk}/msgcenter/SmsTemplate/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/msgcenter/SmsTemplate/show.do?'+$('#pkName').val()+'='+id,'','resizable=yes,location=no,scrollbars');
}
function preview(id){
	window.open('${yk}/msgcenter/SmsTemplate/preview.do?'+$('#pkName').val()+'='+id,'','resizable=yes,location=no,scrollbars');
}
function log(id){
	window.open('${yk}/msgcenter/SmsTemplateLog/list.do?templateId='+id,'','resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/msgcenter/SmsTemplate/edit.do?'+$('#pkName').val()+'='+id,'','resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/msgcenter/SmsTemplate/edit.do?'+$('#pkName').val()+'='+$id.val(),'','resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/msgcenter/SmsTemplate/create.do','','resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/msgcenter/SmsTemplate/delete.do", {"items":ids.join(',')},
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
