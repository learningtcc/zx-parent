...
<head>
	<title><%=EmailAnalyze.TABLE_ALIAS%> 维护</title>
...
<form id="queryForm" name="queryForm" action="<c:url value="/msgcenter/EmailAnalyze/list.do"/>" method="post" style="display: inline;">
	...
	<br></br>
	<div class="title01">
	<h2><%=EmailAnalyze.TABLE_ALIAS%>列表</h2>
</div>
		<div class="groupbtn">
		...
		<div>
     	 ...
     	 <yk:page url="${yk}/EmailAnalyze/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/EmailAnalyze/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/EmailAnalyze/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/EmailAnalyze/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/EmailAnalyze/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids){
	if(ids.length>0 && confirm("确定要删除吗？")){
		$.post( "${yk}/EmailAnalyze/delete.do", {"items":ids.join(',')}, 
				function(data){
					if(data=='1'){
						alert('删除成功');
						$('#queryForm').submit();
