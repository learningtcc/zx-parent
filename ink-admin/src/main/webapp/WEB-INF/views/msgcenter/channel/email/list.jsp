...
<form id="queryForm" name="queryForm" action="<c:url value="/msgcenter/channel/email/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		<tr>	
			...
			<td class="txtr" style="width: 100px">发件服务器：</td>		
			<td>
				&nbsp;<input value="<c:out value="${smtpHost}"/>" id="smtpHost" name="smtpHost" maxlength="40"/>
			</td>

		</tr>	
		<tr>	
			<td class="txtr" style="width: 100px">发送邮箱：</td>		
			<td>
				&nbsp;<input value="<c:out value="${mailAddr}"/>" id="mailAddr" name="mailAddr" maxlength="50"/>
			</td>
		...
		<div>
			...
			<thead>
			 <tr>
				...
				<th>发件服务器<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
				<th>发送邮箱<div style="width: 65px; height: 1px; overflow: hidden;"></div></th>
			...
			<tr>
				...
				<td>
				<div><c:out value="${entry.smtpHost}"/></div>
				</td>
				<td>
				<div><c:out value="${entry.mailAddr}"/></div>
				</td>
				<td>
     	 ...
     	 <yk:page url="${yk}/msgcenter/channel/email/list.do" id="logForm" submitForm="queryForm" joy="true"/>
</div>

<script type="text/javascript">
function show(id){
	window.open('${yk}/msgcenter/channel/email/show.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
function edit(id){
	window.open('${yk}/msgcenter/channel/email/edit.do?'+$('#pkName').val()+'='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}

function statistics(id){
	window.open('${yk}/msgcenter/EmailAnalyze/statistics.do?chnId='+id,'','height=600,width=600,resizable=yes,location=no,scrollbars');
}
...
$(function(){
	...
	$('#btnedit').click(function(){
		...
		window.open('${yk}/msgcenter/channel/email/edit.do?'+$('#pkName').val()+'='+$id.val(),'','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	
	$('#btnadd').click(function(){
		window.open('${yk}/msgcenter/channel/email/create.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
	...
	$("#levelOrder").click(function(){
		window.open('${yk}/msgcenter/channel/email/levelOrder.do','','height=600,width=600,resizable=yes,location=no,scrollbars');
	});
...
function v_deleteItems(ids,status){
	...
	if(ids.length>0 && confirm(message)){
		$.post( "${yk}/msgcenter/channel/email/updateStatus.do", {"items":ids.join(','),"status":status},
				function(data){
					if(data=='1'){
						alert('操作成功');
						$('#queryForm').submit();
