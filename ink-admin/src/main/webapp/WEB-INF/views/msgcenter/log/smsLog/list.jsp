...
<form id="queryForm" name="queryForm" action="<c:url value="/msgcenter/SmsLog/list.do"/>" method="post" style="display: inline;">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table00">
		...
		<tr>
			...
			<td>
				<yk:select codeName="sms_code" table="sms_code" codeValue="sms_code" id="smsCode" name="smsCode" select="${smsCode }" where=" status = ? " paramValue="0" serviceName="msgcenterDubboBaseService"/>
			</td>
...
function show(id, merctCode){
	window.open('${yk}/msgcenter/SmsLog/show.do?'+$('#pkName').val()+'='+id+'&merctCode='+merctCode,'','height=600,width=1000,resizable=yes,location=no,scrollbars');
}
function edit(id){
